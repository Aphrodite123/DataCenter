...
# 生成链接列表
def Get_Url(num):
    urlFront = 'https://rate.tmall.com/list_detail_rate.htm?itemId=602761876641&spuId=1354360687&sellerId=1714128138&order=1&currentPage='
    urlRear = '&append=0&content=1&tagId=&posi=&picture=&groupId=&ua=098%%2FaoF6DpGCvvpvvPMMRphvChCvvvmjvpvhphhvv86CvCCjphQOmpCvojhoSfyT%2Bkt4dphvhhaCt3k1vhCam7uYKLuAbDurvpvpB6vInHhvp3LrFd0tRdUK79wCvva47rMNjmFj9phvHHiaY63gzHi4chNnt1Q4cN14NrGBRphvChCvvvv%3D&needFold=0&_ksTS=1589953317946_3071&callback=jsonp3072'
    for i in range(0, num):
        COMMENT_PAGE_URL.append(urlFront + str(1 + i) + urlRear)


# 获取评论数据
def GetInfo(name, num):
    # 定义需要的字段
    data = [["用户", "类型", "评价", "时间"]]
    nickname = []
    auctionSku = []
    ...
    for i in range(num):
        ...
        for i in list(range(0, len(nickname))):
            data.append([nickname[i], auctionSku[i], ratecontent[i], ratedate[i]])
    ...
    #     with open(r"./data/TmallComments.txt", 'a+', encoding='UTF-8') as file:
    #         file.write(text + ' ')
    # print(i + 1, ":写入成功")
